...
function buildNestedArray(data, showCompleted) {
  ...
  const filteredData = showCompleted ? data : data.filter((item) => item.completed === false || item.completed === null)

  // first pass - populate itemsById with items from filteredData, each having an empty children array
...
function TreeView({
  ...
  useEffect(() => {
    if (goals && goals.length > 0) {
      const tree = buildNestedArray(goals, showCompleted)
      setTreeData(tree)
    }
  }, [goals, showCompleted])
  ...
  const treeLayout = useMemo(() => {
    ...
    return d3
      ...
      .separation((a, b) => (a.parent === b.parent ? 1.2 : 1.5))(hierarchy)
  }, [hierarchy])
